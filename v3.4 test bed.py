        ...
        for sprite in mapMoveBlocks:
            ...
            for sprite in collision_list:
                if self.Y_direct < 0:
                    ...
                    self.Y_direct = 0
                if self.X_direct > 0:
                    # Player is walking to the right of the block
                    self.X_direct = 0
                if self.X_direct < 0:
                    # Player is walking to the left of the bock
                    self.X_direct = 0

        if falling and self.jumpHeight == 0:
            self.rect.y += dp.get("height") / len(maps.mapFiles[currentMap]) / (self.constant / 2)
