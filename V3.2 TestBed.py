...
class Player(pi.sprite.Sprite):
    ...
    def update(self, collidable, speed, mapMoveBlocks):
        for block in mapMoveBlocks:
            if self.dontMoveLeft == False and self.X_direct > 0:
                block.rect.x += self.X_direct
            if self.dontMoveRight == False and self.X_direct < 0:
                block.rect.x += self.X_direct
        ...
        for collided_obj in collision_list:
            if self.X_direct > 0:
                self.dontMoveRight = True
            else:
                self.dontMoveRight = False
            if self.X_direct < 0:
                self.dontMoveLeft = True
            else:
                self.dontMoveRight = False
